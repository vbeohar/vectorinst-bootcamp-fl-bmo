...
def main(config: dict[str, Any], server_address: str, wandb_entity: str | None) -> None:
    ...
    initial_model = AMLNet(input_dim)
    ...
    checkpoint_and_state_module = AdaptiveConstraintServerCheckpointAndStateModule(
        model=initial_model, model_checkpointers=checkpointers
    )
    ...
    server = FedProxServer(
        client_manager=client_manager, fl_config=config, 
        checkpoint_and_state_module=checkpoint_and_state_module,
        strategy=strategy, reporters=reporters, accept_failures=False
    )
